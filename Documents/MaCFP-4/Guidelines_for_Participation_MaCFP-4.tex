...
\input{../MaCFP_common_preamble.tex}
...
Version 1.3 | Released: 19 January 2026\\
\end{small}
\newpage
\thispagestyle{empty}
\tableofcontents
...
\begin{itemize}
 ...
 \item \href{https://github.com/MaCFP/macfp-db/releases/tag/macfp-2.0}{MaCFP-2 (Gas Phase)~\cite{MaCFP2Proceedings}} -- Waterloo, Canada (April, 2021)
 \item \href{https://github.com/MaCFP/matl-db/releases/tag/v1.1.0}{MaCFP-2 (Condensed Phase)~\cite{PreliminarySummary}} -- Waterloo, Canada (April 2021)
...
The MaCFP repository is hosted on GitHub (\url{https://github.com/MaCFP}). Note that the repository is continuously updated, and users are expected to consult the repository regularly for possible additions and/or corrections. This data repository is available to computational groups for fire model calibration and validation. The repository contains:
\begin{itemize}
 ...
 \item An electronic copy of material property sets (i.e., pyrolysis models) calibrated by the different modeling groups, organized in simple comma delimited ASCII files;
...
This guidelines document provides a summary of specific modeling targets of interest for each target experiment to be presented at the workshop and outlines the process for material property calibration. Participants submit their contributions by creating a pull request on Github; details on how to do so (including file formatting requirements) are provided in Secs.~\ref{sec:Radiation}-\ref{sec:Com-Results}. Python-based \href{https://github.com/MaCFP/macfp-db/wiki/Plotting-Scripts}{post-processing tools} are available on the repository. Contributors submit comma-delimited (*.csv) files with their computational results together with another comma-delimited configuration file listing the plots to be made and a Python wrapper script for processing the data files and creating plots. Further details on \href{https://github.com/MaCFP/macfp-db/wiki/How-to-Contribute}{how to contribute} and \href{https://github.com/MaCFP/macfp-db/wiki/Submitting-Computational-Results}{how to submit computational results} are available on the MaCFP/macfp-db wiki.

\newpage
Interested modelers should inform the relevant subgroup of their plans to participate by contacting the respective subgroup Co-Chairs:

\begin{itemize}
\item Morgan Bruns, \href{mailto:mbruns@stmarytx.edu}{mbruns@stmarytx.edu} (Pyrolysis)
\item Isaac Leventon, \href{mailto:isaac.leventon@nist.gov}{isaac.leventon@nist.gov} (Pyrolysis)
\item Bart Merci, \href{mailto:Bart.Merci@ugent.be}{Bart.Merci@ugent.be} (Gas Phase)
\item Arnaud Trouv\'e, \href{mailto:atrouve@umd.edu}{atrouve@umd.edu} (Gas Phase)
\item Fabian Bränström, \href{mailto:braennstroem@uni-wuppertal.de}{braennstroem@uni-wuppertal.de} (Radiation)
\item Simo Hostikka, \href{mailto:simo.hostikka@aalto.fi}{simo.hostikka@aalto.fi} (Radiation)
\end{itemize}
...
\end{itemize}
\item The Gas Phase Subgroup will feature comparisons of experimental data and computational results for three target experiments:
\begin{itemize}
\item Controlled co-flow round diffusion flame experiments (13.7-cm diameter diffusion flames featuring different fuels.
\item Study of flame structure/heat flux and fire growth over PMMA during upward flame spread in two configurations (parallel panels and corner-wall).
\item 1 MW compartment fires producing external flames and spill plumes over a vertical non-combustible façade.
\end{itemize}
\end{enumerate}
...
\\
Spring 2025     & Distribution of material samples for pyrolysis model calibration exercise\\
\\
%\hline
...
\\
May 2026        & Deadline to submit pyrolysis model calibration results \small{(Condensed Phase Subgroup)}\\
...
\item[Phase 2: Radiation simulations without scalar field uncertainty]
...
\item[Phase 3: Radiation simulations with transient CFD]
...
\item Ignition type (spark ignitor, flame, no pilot)
\item Calibration Standard (ISO 5660, ASTM E1354)
\end{itemize}
...
\item CSV (.csv) file(s) containing model predictions of material thermal decomposition behavior that demonstrate proof of model calibration accuracy (e.g., if models were calibrated against mass loss rate measured in the TGA or cone calorimeter, please submit similarly formatted predictions of these datasets)
\end{itemize}

\subsubsection{Target Simulations for Model to Model Comparison}
At the MaCFP-4 workshop, comparisons of pyrolysis model predictions will be performed as:
\begin{enumerate}
...
\hline
Initial Temperature: & Initial ambient and sample temperatures should be 300~K\\
...
\label{sec:Gas Phase}
The Gas Phase Subgroup will coordinate comparisons of experimental data and computational results for the following target experiments:
\begin{itemize}
\item \textbf{FM Burner:} Controlled co-flow round diffusion flame experiments (13.7-cm diameter diffusion flames featuring different fuels -- ethylene as the baseline fuel and also methane, propylene, propane -- and an oxygen-nitrogen oxidizer). Study of soot formation/oxidation and thermal radiation emissions in turbulent buoyant diffusion flames using a burner configuration developed at FM and data also generated at FM. See \url{https://github.com/MaCFP/macfp-db/tree/master/Extinction/FM_Burner}.
\item \textbf{Upward flame spread:} Study of flame structure/heat flux and fire growth over PMMA. Two configurations are considered:
\begin{itemize}
\item A parallel panel configuration based on the FM 4910 fire test and studied at the National Institute of Standards and Technology: flame spread experiments in a 2.45-m parallel panel configuration, with PMMA. See \url{https://github.com/MaCFP/macfp-db/tree/master/Fire_Growth/NIST_Parallel_Panel}.
\item A corner wall configuration based on the EN 13823 Single Burning Item (SBI) fire test and studied at the University of Maryland: flame spread experiments in a 1.5-m corner wall configuration, with PMMA. See \url{https://github.com/MaCFP/macfp-db/tree/master/Fire_Growth/UMD_SBI}.
\end{itemize}
\item \textbf{Compartment/façade fires:} Study of flame structure and heat transfer using a configuration based on the JIS A 1310 fire test and data generated at the Building Research Institute of Japan and the University of Tokyo. See \url{https://github.com/MaCFP/macfp-db/tree/master/Wall_Fires/JIS_Facade}.
\end{itemize}

Interested modeling groups should inform the Gas Phase Phenomena subgroups of their plans to participate in MaCFP-4 by contacting the following Co-Chairs:
...
\vskip.5\baselineskip
Participating modeling groups are asked to submit their computational results by \textbf{May 1, 2026}.  Results will be uploaded on the MaCFP GitHub repository and comparisons between experimental data and computational results will be compiled by the MaCFP organizers in collaboration with experimentalists and modelers.

\subsection{General Modeling Requirements}
Please consider the following:
\begin{itemize}
\item grid convergence study (both gas and solid phase, if necessary)
\item sufficient angular resolution for the radiation solver
\item justification of subgrid modeling choices (e.g., turbulence, combustion, near-wall)
\end{itemize}
...
\subsection{Additional Guidelines for the FM Burner Case}
The FM burner case corresponds to a 13.7-cm-diameter, controlled co-flow (a mixture of air and nitrogen), round ethylene diffusion flame experiment previously studied at FM (\url{https://github.com/MaCFP/macfp-db/tree/master/Extinction/FM_Burner}). The case includes global measurements at coflow oxygen mole fractions of 20.9\%, 19\%, 17\% and 15\%, and local measurements at coflow oxygen mole fraction of 20.9\%, 16.8\% and 15.2\%. Ethylene is the baseline fuel in the data set; some limited data are also available for other fuels: methane, propylene, and propane.
Groups interested in simulating the FM burner case are invited to predict the changes in flame radiation. Simulations with a prescribed radiant fraction (taken as the measured averaged value) are acceptable as well.
\vskip.5\baselineskip
...
\begin{itemize}
\item Plots showing the mean and {\it rms} temperature as a function of radial position $r$, for different elevations $z$;
\item Plots showing the probability density function (PDF) of temperature for different radial positions $r$ and elevations $z$;
\item Plots showing the mean vertical velocity as a function of radial position $r$, for different elevations $z$;
\item Plots showing the mean horizontal velocity as a function of radial position $r$, for different elevations $z$;
\item Plots showing the mean and {\it rms} soot volume fraction as a function of radial position $r$, for different elevations $z$ (for comparisons, use the experimental soot data corresponding to Laser Induced Incandescence -- LII -- measurements);
\item Plots showing the probability density function (PDF) of soot volume fraction for different radial positions $r$ and elevations $z$ (for comparisons, use the experimental soot data corresponding to Laser Induced Incandescence -- LII -- measurements);
...
\item A plot showing the variations of the combustion efficiency with the coflow oxygen mole fraction.
 \end{itemize}

The data required to generate these plots should be provided as ASCII files organized in simple comma-delimited format.

\subsection{Additional Guidelines for the Corner Wall Fire Case}
The corner wall fire case is based on the EN 13823 (SBI) fire test and was previously studied at the University of Maryland. The case corresponds to two (50 × 146) cm2 PMMA plates in a vertical comer wall configuration (\url{https://github.com/MaCFP/macfp-db/tree/master/Fire_Growth/UMD_SBI}). The case includes global heat release rate and local heat flux measurements, including surface heat flux measurements performed on one of the two PMMA plates and external heat flux measurements performed at a certain horizontal distance from the vertical corner connecting the two PMMA plates.

Groups interested in simulating the corner wall fire case are strongly encouraged to use the \href{https://github.com/MaCFP/matl-db/tree/master/PMMA/Material_Properties}{material property sets} developed as part of the MaCFP-2 and MaCFP-3 Workshops.  Specifically, it is suggested that the UMD Pyrolysis model (submitted in 2021) be used by modelers for initial model setup (to provide a common set of parameters for MaCFP PMMA). This property set was identified as yielding the closest predictions (sum of square errors) to gasification experiments used for model validation at the MaCFP-3 Workshop. If modelers wish to use an alternative pyrolysis model, a justification should be provided and they are encouraged to repeat simulations with both property sets.
...
\noindent Suggested angular resolution: $N_{\Omega}$ = 100 angles.
\vskip.2\baselineskip
\noindent Suggested solid-phase grid resolution: $\delta x_s$ at the exposed surface close to 0.1~mm to 0.2~mm.
...
\subsection{Additional Guidelines for the Parallel Panel Fire Case}
The parallel panel fire case is based on the FM 4910 fire test and was previously studied at the National Institute of Standards and Technology (NIST). The case corresponds to two (61~cm × 244~cm) PMMA plates in a vertical parallel panel configuration with a separation distance of 30~cm (\url{https://github.com/MaCFP/macfp-db/tree/master/Fire_Growth/NIST_Parallel_Panel}). In each test, samples were exposed to a propane burner (nominal heat release rate, HRR = 60~kW), which was turned off after sustained flaming was observed across the panel walls. Flames were allowed to spread upward across the panels and continue burning until self-extinction following complete sample burnout. Peak HRR in these experiments measured approximately 3~MW.

The case includes: (1) time-resolved measurements of fire size [kW], soot generation, and gaseous species production (CO and CO$_2$), (2) spatially resolved measurements of flame to wall heat transfer [kW m$^{-2}$] performed along the centerline of the two PMMA plates, and (3) radiative heat flux at a distance [kW m$^{-2}$] performed at a certain horizontal distance from the two PMMA plates.

Groups interested in simulating the parallel panel fire case are strongly encouraged to use the \href{https://github.com/MaCFP/matl-db/tree/master/PMMA/Material_Properties}{material property sets} developed as part of the MaCFP-2 and MaCFP-3 Workshops.  Specifically, it is suggested that the UMD Pyrolysis model (submitted in 2021) be used by modelers for initial model setup (to provide a common set of parameters for MaCFP PMMA). This property set was identified as yielding the closest predictions (sum of square errors) to gasification experiments used for model validation at the MaCFP-3 Workshop. If modelers wish to use an alternative pyrolysis model, a justification should be provided and they are encouraged to repeat simulations with both property sets.
...
Groups are invited to first perform a simulation of the heat transfer induced by the propane burner flame without the PMMA plates. Plots of interest include (all should be submitted for comprehensive comparison between modeling submissions):
\begin{itemize}
\item A plot showing the time variations of the heat release rate;
\item Representative plots showing the instantaneous flame shape (e.g., identified as the 200~kW~m$^{-3}$ iso-contour of the volumetric heat release rate, or using some other method to be specified);
...
\subsection{Additional Guidelines for the Façade Fire Case}
The façade fire case is based on JIS A 1310 fire test and was previously studied at the Building Research Institute of Japan and the University of Tokyo. The case corresponds to a (182 × 409.5) cm2 vertical (inert) wall configuration exposed to flames and combustion gases ejected from a fire compartment through a (91 × 91) cm2 opening (\url{https://github.com/MaCFP/macfp-db/tree/master/Wall_Fires/JIS_Facade}). The case includes local temperature and surface heat flux measurements.

\vskip.5\baselineskip
\noindent Model setup guidelines:
\vskip.2\baselineskip
\noindent Suggested grid resolution: $\Delta x$ from 0.65~cm to 2.5~cm.
\vskip.2\baselineskip
\noindent Suggested angular resolution: $N_{\Omega}$ from 16 to 64 angles.
\vskip.2\baselineskip
\noindent Suggested solid-phase grid resolution: $\delta x_s$ at the exposed surface close to 0.5~mm to 1~mm.

Plots of interest include:
\begin{itemize}
    \item A plot showing the time variations of the fractions of heat release rate occurring inside and outside of the fire compartment;
    \item Representative plots showing the instantaneous flame shape (e.g., identified as the 200-kW/m$^3$ iso-contour of the volumetric heat release rate, or using some other method to be specified);
    \item Plots showing the time variations of the total gauge heat flux (measured by a water-cooled sensor) on the surface of the façade, at a central position $y = 0$ and for different elevations $z$;
    \item A plot showing the vertical variations of the time-averaged net surface heat flux, (possibly including the convective and radiative components of the net surface heat flux), at a central position $y = 0$;
    \item Plots showing the time variations of the temperature (measured by a thermocouple) on the surface of the façade, at a central position $y = 0$ and for different elevations $z$;
    \item A plot showing the vertical variations of the time-averaged temperature on the surface of the façade, at a central position $y = 0$;
    \item Plots showing the spatial variations of the time-averaged gas temperature (measured by a thermocouple), for different horizontal positions x and elevations z;
    \item Plots showing the spatial variations along the x-direction normal to the façade of the mean and rms gas temperature, and the mean and rms vertical flow velocity, at different selected façade locations (i.e., at selected values of $y$ and $z$) and at different selected times.
\end{itemize}
...
For all modeling cases of interest, researchers must clearly communicate key information regarding how models were setup and run (i.e., descriptions of how results were obtained). This README should be submitted as a markdown file with the information requested on the \href{https://github.com/MaCFP/macfp-db/blob/master/Utilities/model_questionnaire.md}{Modeling Questionnaire} (e.g., contributors, CFD model, submodels, etc.).
...
&\\
\hline
FM Burner Case & Gang Xiong (\href{mailto:gang.xiong@fm.com}{gang.xiong@fm.com})\\
Corner Wall Fire Case & Stanislav Stoliarov (\href{mailto:stolia@umd.edu}{stolia@umd.edu})\\
Parallel Panel Fire Case & Isaac Leventon (\href{mailto:isaac.leventon@nist.gov}{isaac.leventon@nist.gov})\\
Façade Fire Case & Xiaoyi Lu (\href{mailto:xiaoyi.lu@fm.com}{xiaoyi.lu@fm.com})\\
