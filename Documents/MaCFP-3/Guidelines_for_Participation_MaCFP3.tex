...
HRR $[$kW$]$& File Format: 2 columns of data (time \& total HRR)\\
	...
	& File Naming Convention: [Institution Name]\_SBI\_HRR.csv \\
&	\\
...
$\dot{q}^{\pp}_{dist}$ $[$kW~m$^{-2}$$]$& File Format: 8 columns of data (time \& total heat flux at 7 different heights above top of burner)\\
	...
	& Duration: Minimum of 200 s or 300 kW (whichever is greater) \\
	& File Naming Convention: [Institution Name]\_SBI\_q\_wall.csv \\
&	\\
...
$\dot{q}^{\pp}_{wall}$ $[$kW~m$^{-2}$$]$& File Format: each file contains 5 columns of data (time \& total flame heat flux, at a given height, at 4 locations away from the corner wall)\\
	& Resolution: Time-resolved predictions at 5~s intervals \\
	& Duration: Minimum of 200 s or 300 kW (whichever is greater) \\
	& File Naming Convention: [Institution Name]\_SBI\_q\_distance.csv \\
\hline
\end{tabular}
\end{table}
...
$\dot{q}^{''}_{wall}$ $[$kW~m$^{-2}$$]$& File Format: 10 columns of data (HRR \& total flame heat flux, at the centerline of panel walls, at 9 heights, $z$, above the base of samples )\\
	...
	& File Naming Convention: [Institution Name]\_Parallel-Panel\_q\_wall.csv \\
\hline
\end{tabular}
\end{table}
...
\subsection{Modeling Results: Calibrated Material Property Datasets and Simulations of Fire Behavior; (.json and .csv files)}
...
Modelers are asked to provide simulations of one or both of these experiments using material property sets developed by the participants of the condensed-phase portion of this working group. Final submissions should be provided by \textcolor{red}{**DATE}. \\
