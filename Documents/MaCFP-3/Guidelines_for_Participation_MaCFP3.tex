...
\begin{itemize}[noitemsep]
\item Morgan Bruns, \href{mailto:mbruns@stmarytx.edu}{mbruns@stmarytx.edu} (Co-Chair of the Condensed Phase Phenomena subgroup) (EXPT3, EXPT4, EXPT5)
\item Isaac Leventon, \href{mailto:isaac.leventon@nist.gov}{isaac.leventon@nist.gov} (Co-Chair of the Condensed Phase Phenomena subgroup) (EXPT3, EXPT4, EXPT5)
\item Bart Merci, \href{mailto:Bart.Merci@ugent.be}{Bart.Merci@ugent.be} (Co-Chair of the Gas Phase Phenomena subgroup) (EXPT1, EXPT2, EXPT4, EXPT5)
\item Arnaud Trouv\'e, \href{mailto:atrouve@umd.edu}{atrouve@umd.edu} (Co-Chair of the Phase Gas Phenomena subgroup) (EXPT1, EXPT2, EXPT4, EXPT5)
 \end{itemize}
...
\begin{itemize}[noitemsep]
\item \href{https://github.com/MaCFP/macfp-db/tree/master/Liquid_Pool_Fires/NIST_Pool_Fires}{EXPT1: Liquid pool fire experiments} (30 cm and 100 cm diameter pool flames studied at NIST and featuring multiple fuels).
\item \href{https://github.com/MaCFP/macfp-db/tree/master/Extinction/FM_Burner}{EXPT2: Controlled co-flow round diffusion flame experiments} (13.7 cm diameter ethylene diffusion flames studied at FM Global and featuring an oxygen-nitrogen oxidizer).
\item \href{https://github.com/MaCFP/matl-db/tree/master/PMMA/Validation_Data/NIST_Gasification_Apparatus}{EXPT3: Benchmark gasification experiments of MaCFP-PMMA} (bench-scale thermal degradation experiments conducted in the NIST gasification apparatus, providing validation data for PMMA pyrolysis models).
\item \href{https://github.com/MaCFP/macfp-db/tree/master/Fire_Growth/UMD_SBI}{EXPT4: Flame spread experiments in a 1.46~m corner wall configuration} (Single Burning Item experiments studied at UMD with MaCFP-PMMA).
\item \href{https://github.com/MaCFP/macfp-db/tree/master/Fire_Growth/NIST_Parallel_Panel}{EXPT5: Flame spread experiments in a 2.44~m parallel panel configuration} (parallel panel experiments studied at NIST with MaCFP-PMMA).
 \end{itemize}
...
\subsection{EXPT1: Pool Fires}
\href{https://github.com/MaCFP/macfp-db/tree/master/Liquid_Pool_Fires/NIST_Pool_Fires}{EXPT1 (Liquid pool fire experiments)} corresponds to 30-cm and 100-cm  diameter pool flames studied at NIST. The MaCFP experimental database also considers a 30-cm-diameter acetone pool fire, a 30-cm-diameter ethanol pool fire, a 37-cm diameter methane fire, and a 37-cm diameter propane fire. Groups interested in simulating several of these configurations are invited to focus on the 30-cm and 100-cm methanol cases first. Because of time constraints, discussions at MaCFP-3 may be limited to these methanol pool fire cases.
 
Groups interested in simulating EXPT1 are invited to focus first on simulations with a prescribed liquid evaporation rate (taken as the measured averaged value). Simulations with an evaporation rate that is calculated as a function of the thermal feedback are welcome but should be considered as a second step. Similarly, groups interested in simulating EXPT1 are invited to focus first on simulations with a prescribed global radiant fraction (taken as the measured averaged value). Simulations that simulate rather than prescribe the radiant fraction are welcome but should be considered as a second step.

Suggested grid resolution (30 cm pool): $\Delta x$ from 0.5 cm to 6 cm.

Suggested grid resolution (100 cm pool): $\Delta x$ from 1 cm to 20 cm.

Suggested angular resolution: $N_{\Omega}$ = 100 angles.\\
...
\subsection{EXPT2: Controlled co-flow round diffusion flame experiments}
\href{https://github.com/MaCFP/macfp-db/tree/master/Extinction/FM_Burner}{EXPT2 (Controlled co-flow round diffusion flame experiments)} corresponds to a 13.7-cm-diameter, controlled co-flow (a mixture of air and nitrogen), round ethylene diffusion flame experiment currently studied at FM Global. EXPT2 includes global measurements at coflow oxygen mole fractions of 20.9\%, 19\%, 17\% and 15\%, and detailed measurements at coflow oxygen mole fraction of 20.9\%, 16.8\% and 15.2\%. Note that the current experimental database does not include the oxygen extinction limit of the flame and focuses instead on the change in the flame radiation observed as this limit is approached.

Groups interested in simulating EXPT2 are invited to predict the changes in flame radiation. Simulations with a prescribed radiant fraction (taken as the measured averaged value) are acceptable as well.

Suggested grid resolution: $\Delta x$ from 0.5 cm to 2 cm.
...
\subsection{EXPT3: Experimental Data for Pyrolysis Model Validation}
...
\subsection {EXPT4: Single Burning Item (SBI) Tests}
...
\FloatBarrier
\subsection {EXPT5: Parallel Panel Tests}
...
\begin{itemize}[noitemsep]
 \item EXPT1 (Liquid pool fire experiments): Anthony Hamins (\href{mailto:anthony.hamins@nist.gov}{anthony.hamins@nist.gov})
 \item EXPT2 (Controlled co-flow round diffusion flame experiments): Yi Wang (\href{mailto:yi.wang@fmglobal.com}{yi.wang@fmglobal.com})
 \item EXPT4 (Flame spread experiments in a 1.5-m corner wall configuration): Stanislav Stoliarov (\href{mailto:stolia@umd.edu}{stolia@umd.edu})
 \item EXPT3 (Benchmark gasification experiments of MaCFP-PMMA) and EXPT5 (Flame spread experiments in a 2.44-m parallel panel configuration): Isaac Leventon (\href{mailto:isaac.leventon@nist.gov}{isaac.leventon@nist.gov})
 ...
 \item General questions on MaCFP, Bart Merci (Co-Chair, \href{mailto:bart.merci@ugent.be}{bart.merci@ugent.be}) and Arnaud Trouv\'e (Co-Chair, \href{mailto:atrouve@umd.edu}{atrouve@umd.edu})	
\end{itemize}
 
\footnotesize{\bibliography{{../MaCFP_References.bib}}}
