...
Experimental and Modeling Results (Fire Growth Experiments) will be submitted, stored, and made publicly available in the appropriate subfolder of the \href{https://github.com/MaCFP/}{MaCFP GitHub Repository}. Details on how to contribute are available on the \href{https://github.com/MaCFP/macfp-db/wiki/How-to-Contribute}{MaCFP-db wiki}. Results can be submitted by creating a pull request on Github; please ensure that your results are formatted as per the guidelines below. Template files (e.g., .json files holding material property information or .csv files of model predictions) for all requested numerical simulations are available on the Github repository. Sumbissions of numerical simulation results should match the file format, data structure, naming convention (file and data names), and reported units of available validation data. Note, some iteration on formatting may be required before the results can be merged into the MaCFP database. %For simplicity, please collect your files in a single folder with your INSTITUTE name.
