#!/bin/bash
# Configure Python Virtual Environment for MaCFP
...
reporoot=$(pwd)
cd "$reporoot/macfp-db/.github" || error_exit "Directory not found: $reporoot/macfp-db/.github"

VENV_DIR="macfp_python_env"
...
export PYTHONPATH="$reporoot/macfp-db/Utilities:$PYTHONPATH"

# Run test script to verify environment
cd "$reporoot/macfp-db/Utilities" || error_exit "Failed to find script directory"
