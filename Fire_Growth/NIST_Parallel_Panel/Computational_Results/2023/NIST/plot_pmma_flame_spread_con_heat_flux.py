...
M1 = pd.read_csv('Preliminary_Results/PMMA_60_kW_1_cm_devc.csv', sep=',', skiprows=1)
M2 = pd.read_csv('Preliminary_Results/PMMA_60_kW_2_cm_devc.csv', sep=',', skiprows=1)
M5 = pd.read_csv('Preliminary_Results/PMMA_60_kW_5_mm_devc.csv', sep=',', skiprows=1)
...
for irow in range(len(hrr_label)):
    ...
    x1 = M1.loc[indices1[irow],"CHF-1":"CHF-50"].values[:].astype(float)
    x2 = M2.loc[indices2[irow],"CHF-1":"CHF-50"].values[:].astype(float)
    x5 = M5.loc[indices5[irow],"CHF-1":"CHF-50"].values[:].astype(float)

    f = macfp.plot_to_fig(x_data=x5, y_data=z_FDS, data_label=hrr_label[irow],
                          x_min=0,x_max=20,x_nticks=5,
                          y_min=0,y_max=250,
                          x_label='Convective Heat Flux [kW/m²]',
                          y_label='Height [cm]',
                          marker_style='None',
                          line_style='-',
...
plt.savefig('Preliminary_Results/Plots/PMMA_flame_spread_conheatflux.pdf')
