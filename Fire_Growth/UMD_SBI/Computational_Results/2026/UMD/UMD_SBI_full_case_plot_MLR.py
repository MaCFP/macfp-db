...
Example::
  ...
  python3 UMD_SBI_full_case_plot_MLR.py --tmax 200
"""
...
CASES: list[tuple[str, str]] = [
    ("4 cm", "UMD_SBI_full_case_4_cm_devc.csv"),
    ("2 cm", "UMD_SBI_full_case_2_cm_devc.csv"),
    ("1 cm", "UMD_SBI_full_case_1_cm_devc.csv"),
    ("0.5 cm", "UMD_SBI_full_case_0p5_cm_devc.csv"),
]
...
def load_all(data_dir: Path) -> list[tuple[str, np.ndarray, np.ndarray, np.ndarray, np.ndarray]]:
    ...
    for label, fname in CASES:
        ...
        loaded.append((label, t, y0, x0, tot))
    return loaded
...
def plot_mlr(data_dir: Path, png_out: Path, png_tot_out: Path, tmax: float | None) -> None:
    ...
    for ax, (title, getter) in zip(axes, titles):
        for i, (label, t, y0, x0, tot) in enumerate(loaded):
    ...
    if tmax is not None:
        axes[-1].set_xlim(0.0, float(tmax))
    ...
    for i, (label, t, _y0, _x0, tot) in enumerate(loaded):
        m = np.ones(len(t), dtype=bool)
        if tmax is not None:
            m &= t <= float(tmax)
...
def main() -> None:
    ...
    plot_mlr(args.data_dir.resolve(), args.plot.resolve(), args.plot_tot.resolve(), args.tmax)
